...
c1.metric("Totaal weergaven",f"{int(monthly['Weergaven'].sum()):,}".replace(",","."))
c2.metric("Gem. engagement (nieuw)",f"{avg_new:.1f}%",delta=f"{avg_new-avg_old:+.1f}% vs baseline")
c3.metric("Beste maand",monthly.loc[monthly['Weergaven'].idxmax(),'Maand'],f"{int(monthly['Weergaven'].max()):,} views".replace(",","."))
c4.metric("Posts totaal",len(df_posts))
st.markdown("---")
...
if "👥 Volgers" in tm:
    with tm["👥 Volgers"]:
        ...
        fig_f=go.Figure()
        fig_f.add_trace(go.Scatter(x=fol_growth["Datum"],y=fol_growth["Cumulatief"],fill="tozeroy",line=dict(color=ORANGE,width=2),fillcolor=f"{ORANGE}22",name="Cumulatief"))
